...
# def Get_Index_of_SAS_prime(s,a,s_prime):
#     for i in range(len(ListOf_SAS_prime_reward)):
...
# Check if a winning move is available for the user ("O")
    for row in range(3):
        for col in range(3):
            if board[row][col] == ' ':
                board[row][col] = 2  # Assume the user is "O"
                if IsWin(board, 2):
                    return row, col
    ...
    for row in range(3):
        for col in range(3):
            if board[row][col] == ' ':
                board[row][col] = 1  # Assume the opponent is "X"
                if IsWin(board, 1):
                    return row, col
                board[row][col] = ' '  # Undo the move
    move = 0
    while True: # repeat until a valid move for user is found
...
def choose_optimal_move(board):
    ...
    for row in range(3):
        for col in range(3):
            if board[row][col] == ' ':
                board[row][col] = 1  # Assume the user is "O"
                if IsWin(board, 1):
                    return row, col
                board[row][col] = ' '  # Undo the move
    ...
    for row in range(3):
        for col in range(3):
            if board[row][col] == ' ':
                board[row][col] = 2  # Assume the opponent is "X"
                if IsWin(board, 2):
                    return row, col
                board[row][col] = ' '  # Undo the move
    move = 0
...
for i in range(1000): # no of episodes
    ...
    while True: # this loop terminates when a episode completes
        ...
        # #if player is user for learing purpose lets make its move random
        #     move = 0
        #     while True: # repeat until a valid move for user is found
        #         move = availableMove[random.randint(0, 8)] # select a random index from available moves
        #         if(IsValidMove(state,move)):
        ...
        #     state = copy.deepcopy(state) # updating current state with a deepcopy of state
        ...
        if(player == 1): # player is user
            ...
            states.append(state) # pushing the newly created state in the List of all states
            player = 3-player # switching to Agent
            state = copy.deepcopy(state) # updating current state with a deepcopy of stat
            ...
            if(IsWin(state,player)):
                ListOf_SAS_prime_reward.append( [states[-1],move,state,1])
                ...
                # ListOf_SAS_prime_reward.append( [states[-1],move,state,-0.5])
                ListOf_SAS_prime_reward.append( [states[-1],move,state,0.5])
                isEpisodeComplete = True
            #  for intermediate state reward is 0
            ListOf_SAS_prime_reward.append( [states[-1],move,state,0.1]) # here states[-1] is thee last eleent i.e our s on which we perform move and get state in which agent played its turn
...
# You can normalize the counts to get probabilities
...
def value_iteration(transition_probabilities, rewards, gamma=0.9, epsilon=1e-6):
    ...
    while True:
        ...
        for (s, a), s_primes in transition_probabilities.items():
            old_v = V[s]
            # q_values = {s_prime: rewards.get((s, a, s_prime), 0) + gamma * V[s_prime] for s_prime in s_primes}
            # q_values = {s_prime: rewards.get((s, a, s_prime), 0) + gamma * V[s_prime] for s_prime in s_primes}
            ...
            # max_q_value_state = max(q_values, key=lambda k: q_values[k][0])
            # V[s] = max_q_value
            V[s] = max_q_pair[1][0]
            # optimal_actions[s] = max(q_values, key=q_values.get)
...
# # Convert state tuples to lists for better dictionary compatibility
# optimal_actions = {list(state): action for state, action in optimal_actions.items()}

# # Print the optimal actions
# for state, action in optimal_actions.items():
#     print(f"State: {state}, Optimal Action: {action}")

# # If you want to return the optimal_actions dictionary from the function, you can replace the print statement with:
# # return optimal_actions
